...
#define PORT 25565       // portul pe care va asculta conexiuni
...
typedef struct
{
    int jucator1;
    int jucator2;
    char name1[50]; // nume jucator 1
    char name2[50]; // nume jucator 2
...
// stocam in coada atat socket-ul, cat si numele jucatorului
typedef struct
{
    int socket;
    char name[50];
...
// functie care trateaza cazul de remiza
...
// functie care verifica daca mutarea trimisa este valida
...
// thread folosit pentru gestionarea jocului
void *joc_thread(void *arg)
{
    ...
    // initializare tabla (de fiecare data cand incepe un joc nou)
    init_tabla(joc->tabla);

    // afisam la inceput cine joaca: nume jucator1 (X) si jucator2 (O)
    memset(buffer, 0, sizeof(buffer));
    sprintf(buffer, "Incepe jocul intre %s (X) si %s (O)!\n", joc->name1, joc->name2);

    // trimitem mesajul la ambii jucători
    ...
    while (1){
        ...
        // trimitem tabla ambilor jucatori
        memset(joc_str, 0, sizeof(joc_str));
        afisare_tabla(joc->tabla, joc_str);

        send(jucator_curent_fd, joc_str, sizeof(joc_str), 0);
        send(jucator_oponent_fd, joc_str, sizeof(joc_str), 0);

        // mesajele aferente jucatorilor
        ...
        while (1){  
            ...
            if ((bytes = recv(jucator_curent_fd, mutare, sizeof(mutare), 0)) <= 0){
        ...
        if (verifica_castigator(joc->tabla)){
            ...
            // jucatorul curent vede tabla + mesaj de castig
            ...
            if (joc->jucator_curent == 1)
            {
                ...
                // oponentul vede tabla + mesaj de defeat
        ...
        {
            // verificam daca jocul s-a terminat cu remiza
            afisare_tabla(joc->tabla, joc_str);
            send(jucator_curent_fd, joc_str, strlen(joc_str), 0);
            send(jucator_oponent_fd, joc_str, strlen(joc_str), 0);
...
// thread pentru matchmaking care asociaza jucatorii in perechi
...
int main()
{
    ...
    // configuram socketul pentru a permite reutilizarea adresei
    ...
    // asociem socketul cu adresa si portul specificat
    ...
    // cream un thread separat pentru matchmaking
