...
void joaca(int sock)
{
    ...
    while(1)
    {
        // asteptam tabla
        memset(buffer, 0, sizeof(buffer));
        if ((bytes = recv(sock, buffer, 10, 0)) <= 0)
        {
        ...
        if (strstr(buffer, "castigat") || strstr(buffer, "pierdut") || strstr(buffer, "Remiza") || strstr(buffer, "deconectat") || strstr(buffer, "forfeit"))
        {
            break;
        }
        ...
        if (strstr(buffer, "Introduceti mutarea") || strstr(buffer, "randul tau"))
        {
            ...
            // trimitem la server
            send(sock, move, strlen(move), MSG_CONFIRM);
        }
    }  
}
...
int main()
{
    ...
    // trimitem numele jucatorului imediat dupa conectare
    char name[50];
